...
@export_storage var color_multipliers: Projection = Projection.IDENTITY
...
static func apply(first: TextureAtlasColorMatrix, second: TextureAtlasColorMatrix) -> TextureAtlasColorMatrix:
	var matrix: TextureAtlasColorMatrix = first.duplicate()
	matrix.color_multipliers *= second.color_multipliers
	matrix.color_offsets += second.color_offsets
	return matrix
